...
data_register_slave = {
    ...
    40012: {"master": -1, "func": 0x03, "act": {"pack": ">h", "scale": 1, "value": 0}},
    # solax ask from: func 03, 8+1 = offset 40009
    40009: {"master": -1, "func": 0x03, "act": {"pack": ">h", "scale": 1, "value": 0}},
    48193: {"master": 48193, "func": 0x03, "act": None},
}
...
peer = b"\x94\xb9\x7e\xd9\x99\x6c"  # Other ESP mac address
...
class Runner:
    ...
    async def _activate(self):
        ...
        self.e_lan = (
            aioespnow.AIOESPNow()
        )  # Returns AIOESPNow enhanced with async support 24:6F:28:04:80:64 24:6F:28:04:71:1C
    ...
    async def espnow_meter_server(self):
        async for mac, msg in self.e_lan:
            ...
            try:
                ...
                remote_unit_addr, remote_reg_func, remote_reg_addr = struct.unpack_from(
                    ">BBH", msg, 0
                )
                reg_offset = reg_code[remote_reg_func] + remote_reg_addr
                # DEBUG
                log.debug(
                    f"Remote: unit_addr: {remote_unit_addr}, reg_func: {remote_reg_func}, reg_addr: {remote_reg_addr}"
                )
                ...
                if crc:
                    unit_addr, reg_func, byte_qty = struct.unpack_from(
                        "BBB", modbus_pdu, 0
                    )
                    # DEBUG
                    log.debug(
    ...
    def panel_request_decode(self, request):
        ...
        if crc:
            # Request param
            unit_addr, reg_func, reg_addr, qty = struct.unpack_from(
                ">BBHH", request_data, 0
            )  # 00: 00 : 00 00 : 00 00
            # DEBUG
            log.debug(
                f"   addr: {unit_addr}, func: {reg_func}, reg_addr: {reg_addr}, qty: {qty} "
            )
                ...
                if master_offset in data_register_master:
                    ...
                    if data_master["alive"] >= 5:
                        ...
                        if data_slave["act"] is not None:
                            value_byte = self._act(
                                data_master["value"], **data_slave["act"]
                            )
